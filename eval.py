...
def run_eval(args):
  ...
  if not args.tacotron_perf_only:
    # default, run with grifflim-layers to generate wav
    ...
    # for tacotron performance test, run without grifflim-layers
    ...
    for i in range(steps):
      # for tacotron performance test, select 1st str to generate 5s mel spectrum
      text = sentences[0]
    ...
    # mel spectrum len: use the mel spectrum can genertate 5s wav
    print('Mel spectrum len: {0} s'.format(str(mel_spectrum_len)))
    # latency: generate 5s mel spectrum cost how much session run time
    print('Latency: {0:.4f} ms'.format(time_average * 1000))
    # throughput: generate how long mel spectrum per second
...
def main():
  ...
  parser.add_argument('--tacotron-perf-only',
                      help='Choose to only measure tacotron performance'
                           'without grifflim layers',
                      action='store_true', dest='tacotron_perf_only')
